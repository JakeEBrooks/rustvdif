...
impl VDIFVTP {
    ...
    pub fn recv_frame(&mut self) -> Result<(u64, VDIFFrame)> {
        ...
        unsafe {
            ...
            self.sock.recv(std::slice::from_raw_parts_mut(vtp_frame_buf.as_mut_ptr() as *mut u8, self.frame_size + 8))?;
            // Reinterpret all but the first u64 as u32s and copy them to a new VDIF frame.
            out_frame = VDIFFrame::from_slice(std::slice::from_raw_parts((vtp_frame_buf.as_ptr().add(1)) as *const u32, self.frame_size/4));
        }
...
impl VDIFOrderedVTP {
    ...
    pub fn new<A: ToSocketAddrs>(addr: A, frame_size: usize) -> Result<Self> {
        ...
        return Ok(Self { vdifvtp: vdifvtp, expecting_frame: 0 })
    }
